...
class MainWin: #程序主窗
    ...
    def insert(self, text, end="\n"): #向运行展示框加入文字
        self.Text.config(state=NORMAL)
        self.Text.insert(END, f"{text}{end}")
    ...
    def start(self): #开始搜索答案
        try:
            ...
            if len(strings) == 0: #没在屏幕内匹配到基准图标，即没找到题目
                ...
                for one in gen:
                    if one != "Start":
                        self.insert(f"[搜索 反馈] {one}")
                    if count < len(strings):
                        self.insert(f"----↓[{count+1}/{n}]↓----")
                        self.insert(f"[识别 结果] {strings[count]}")
                        count += 1
                self.insert("-------- DONE --------")
                        
        except Exception as err:
            self.insert(f"[ Error  ] {err}")
...
class SetWin: #Selenium配置窗口
    def __init__(self, creat=False):
        ...
        if not pl.release() == "7":
            ...
            self.root.tk.call('tk', 'scaling', ScaleFactor/75)
        self.SF = ScaleFactor/100
        self.root.geometry("{}x{}".format(int(600*self.SF),int(300*self.SF)))
        ...
        self.TesseractOCR_path = StringVar(master=self.root)
        ...
        if os.path.isfile(DATA_PATH): #如已有数据
            ...
            self.TesseractOCR_path.set(dic["TesseractOCR_path"])
    ...
    def widget(self, browser_chose=0): #browser_chose:浏览器选项卡默认选项
        ...
        self.Lf3 = LabelFrame(self.root, text="Tesseract-OCR配置", labelanchor="nw")
        Label(self.Lf3, text="选择Tesseract-OCR文件夹", font=font).grid(row=0, column=1, ipadx=10, padx=(85,0), pady=(0,10))
        self.Et3 = Entry(self.Lf3, textvariable=self.TesseractOCR_path, width=21, font=font, state="readonly")
        self.Et3.grid(row=0, column=3, padx=(25,10), pady=(0,10))
        self.Bt3 = Button(self.Lf3, text="选择", command=self.__choose_TesseractOCR)
        self.Bt3.grid(row=0, column=4, pady=(0,10))
        self.Lf3.pack(fill=X, padx=(13,13), pady=(15,20))
    ...
    def __choose_TesseractOCR(self):
        path = self.__choose_file("选择Tesseract-OCR")
        self.TesseractOCR_path.set(path)
        self.root.update()
    ...
    def __yesBt(self): #确认按钮事件
        ...
        TesseractOCR_path = self.TesseractOCR_path.get()
        if "" in [browser, browser_path, TesseractOCR_path]: #浏览器\浏览器路径存在缺失
            flag = False
        ...
        if not pluginunitAutoMode: #未选择自动匹配浏览器插件
            ...
            if pluginunitPath == "": #浏览器插件路径缺失
                flag = False
        else: pluginunitPath = None

        if flag:
            self.__output(browser, browser_path, pluginunitPath, TesseractOCR_path) #写入文件
        else: self.__err("输入错误", "数据缺失")
            
    def __err(self, title, message):
        msb.showerror(title=title, message=message)

    def __output(self, browser:str, browser_path:str, plugin_unit, TesseractOCR_path:str):
        dic = {"browser":browser, "browser_path":browser_path, "plugin_unit":plugin_unit, "TesseractOCR_path":TesseractOCR_path}
...
if __name__ == "__main__":
    ...
    if os.path.isfile(DATA_PATH):
        ...
        msb.showinfo(title="提示", message="未设置参数")
